...
Application.init = function (opts) {
    ...
    process.on('SIGINT', function () {
        logger.warn('server closed by hand!');
        self.components['monitor'].stop(() => {
        });
    });
    process.on('SIGTERM', function () {
        self.set(Constants.RESERVED.SERVER_STATE, Constants.SERVER_STATE.TERMINATING);
 ...
 * @memberOf Application
 */
Application.setOnStop = function (fn) {
    this.onStop = fn;
};
...
Application.afterStart = function (cb) {
    ...
    appUtil.optComponents(this.loaded, Constants.RESERVED.AFTER_START, function (err) {
        ...
        self.set(Constants.RESERVED.SERVER_STATE, Constants.SERVER_STATE.NORMAL);
