...
Monitor.prototype.getServersInRedis = async function () {
    ...
    let serverResults = this.client.mget(keys);
    let servers = {};
...
Monitor.prototype.updateServerInfo = function (force) {
    ...
    if (force) {
        ...
        this.client.sadd(this.setKey, selfServer.id);
    } else {
        this.client.sismember(this.setKey, selfServer.id).then(selfInRedis => {
            if (!selfInRedis) {
        ...
        this.client.get(key).then(selfKeyInfo => {
            if (!selfKeyInfo) {
                ...
                if (selfKeyInfo.state === selfKeyInfo.state) {
