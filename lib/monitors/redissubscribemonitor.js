...
Monitor.prototype.start = async function (cb) {
    ...
    this.subscriber.on('connect', function () {
        ...
        self.subscriber.config("get", "notify-keyspace-events").then(config => {
            if (config !== 'g$xeK') {
                logger.error("redis config notify-keyspace-events is not g$xeK, correcting...");
                let result = self.subscriber.config("set", "notify-keyspace-events", "g$xeK");
                if (!result) logger.error("set redis notify-keyspace-events to g$xeK failed");
            }
        });
