...
const Monitor = function (app, opts) {
    ...
    this.hashKey = opts.hashKey || "pomelo_servers";//redis_hash_key 请务必不是由redis_key的前缀开头
...
Monitor.prototype.start = async function (cb) {
    ...
    this.subscriber.on('connect', function () {
        ...
        self.subscriber.config("get", "notify-keyspace-events").then(config => {
            if (config[1] !== 'g$xeK') {
                logger.error("redis config notify-keyspace-events is not g$xeK", config[1]);
...
Monitor.prototype.stop = function (cb) {
    ...
    this.client.hdel(this.hashKey, this.app.getCurServer().id).then().catch();
...
Monitor.prototype.getServersInRedis = async function () {
    let serverResults = await this.client.hgetall(this.hashKey);
    let servers = {};
...
Monitor.prototype.updateServerInfo = function (force) {
    ...
    this.client.hget(this.hashKey, selfServer.id).then(selfInRedis => {
        ...
        this.client.set(key, JSON.stringify(selfServer), 'EX', this.expire).catch(e => {
            logger.error(e);
        });
        this.client.hset(this.hashKey, selfServer.id, JSON.stringify(selfServer)).then(res => {
        }).catch(e => {
            logger.error(e);
        });
    }).catch(e => {
