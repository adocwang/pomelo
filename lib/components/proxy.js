...
var getProxyRecords = function(app, sinfo) {
  ...
  if (record) {
    records.push(pathUtil.remotePathRecord('sys', sinfo.serverType, record, sinfo.id));
  }

  // user remote service path record
  record = pathUtil.getUserRemotePath(appBase, sinfo.serverType);
  if (record) {
    records.push(pathUtil.remotePathRecord('user', sinfo.serverType, record, sinfo.id));
  }
