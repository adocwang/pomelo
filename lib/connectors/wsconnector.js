...
Connector.prototype.start = function (cb) {
    ...
    if (this.port === 80 || this.port === 8000 || this.port === 8080) {
        delete options.key;
        delete options.cert;
    ...
    this.opts.server.on('upgrade', async function upgrade(request, socket, head) {
        if (self.terminating) {
            socket.destroy();
            return;
        }

        self.wsServer.handleUpgrade(request, socket, head, function done(ws) {
            self.wsServer.emit('connection', ws, request);
        });
    });
    ...
    this.wsServer.handleUpgrade(request, socket, head, function done(ws) {
        this.wsServer.emit('connection', ws, request, ...args);
    });
        ...
        for (let id in self.wsSockets) {
            ...
            try {
                if ((nowTime - wsSocket.lastActiveTime) > self.opts.heartbeatTimeout) {
                    ...
                    // delete self.wsSockets[wsSocket.id];
                    wsSocket.disconnect(1001, "ping time out");
                }
            } catch (e) {
                console.error(e);
            }
