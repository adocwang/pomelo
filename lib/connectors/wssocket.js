...
var Socket = function (id, socket, remoteAddress) {
    ...
    socket.on('pong', function (event) {
        ...
        if (process.env.WS_DEBUG) {
            console.warn("onPooooong id: " + self.id + " time:" + (new Date()).getTime());
        }
    });
    socket.on('ping', function (event) {
        self.lastActiveTime = (new Date()).getTime();
        if (process.env.WS_DEBUG) {
            console.warn("onPiiiiing id: " + self.id + " time:" + (new Date()).getTime());
        }
    });
...
Socket.prototype.ping = function (data) {
    ...
    if (process.env.WS_DEBUG) {
        console.warn("onPiiiiing id: " + this.id + " time:" + (new Date()).getTime());
    }
